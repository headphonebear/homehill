...
**⚠️ IMPORTANT:** The `storage` Application uses an umbrella chart with Helm dependencies. ArgoCD needs time to download the Longhorn chart from `https://charts.longhorn.io` and render it with your `values.yaml` (which uses the `longhorn:` prefix for dependency values).

**Wait for storage Application to sync:**

```bash
# Wait for Helm dependency resolution and initial sync (may take 2-3 minutes)
kubectl wait --for=condition=Synced application/storage -n argocd --timeout=300s
```

> **WHY:** Without this wait, the first `get application` may show transient errors during Helm chart download. This wait ensures the dependency is resolved before proceeding.
